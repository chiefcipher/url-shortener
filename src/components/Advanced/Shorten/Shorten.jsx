    ...
    return ( 
        <div className="shorten" > 
            ...
            {props.shortenedURL.map ( item => (
                <li key={item.id} className="shorten__url" > <div className="main-url">{item.mainURL}</div><div className="shortened-url">{item.shortenedURL}</div> <button className="copy-btn" onClick={()=> props.clickCopy(item.id)}>{item.statusText} </button></li>
